...
import org.apache.flink.contrib.streaming.state.OutOfCoreKvState.SnapshotWrapper;
...
public class HdfsStateTest {
	...
	private static void put(HdfsKvState<Integer, Integer, String> state, Integer key, String value) throws IOException {
		state.setCurrentKey(key);
		state.setCurrentNamespace(1);
		state.update(value);
	}

	private static String get(HdfsKvState<Integer, Integer, String> state, Integer key) throws IOException {
		state.setCurrentKey(key);
		state.setCurrentNamespace(1);
		return state.value();
	}
	...
	private static void runTest(CheckpointerFactory cf, boolean keepLocal, boolean copyBack)
		...
		backend.initializeForJob(new DummyEnvironment("test-task", 2, 1), "", IntSerializer.INSTANCE);

		FileSystem fs = backend.getLocalFileSystem();

		ValueStateDescriptor<String> vs = new ValueStateDescriptor<String>("t", String.class, "");
		vs.initializeSerializerUnlessSet(null);

		final HdfsKvState<Integer, Integer, String> state = new HdfsKvState<>(
				backend,
				"test",
				conf,
		...
		SnapshotWrapper<Integer, Integer, String, FsStateBackend> s = (SnapshotWrapper<Integer, Integer, String, FsStateBackend>) state
				.snapshot(1000, 1000);
		...
		b.initializeForJob(new DummyEnvironment("", 1, 0), "", IntSerializer.INSTANCE);

		state.dispose();
		HdfsKvState<Integer, Integer, String> state2 = (HdfsKvState<Integer, Integer, String>) s.restoreState(b,
				IntSerializer.INSTANCE, Thread.currentThread().getContextClassLoader(), 1500);
	...
	public void testMerge(CheckpointerFactory cf, boolean keepLocal)
		...
		try (HdfsCheckpointManager checkpointManager = new HdfsCheckpointManager(cpFs, cpDir, localFs, tmpDir,
				new HashMap<Interval, URI>(),
				conf)) {
			Map<Tuple2<Integer, Integer>, Optional<Integer>> kv = new HashMap<>();
			kv.put(Tuple2.of(0, 0), Optional.of(0));
			kv.put(Tuple2.of(0, 1), Optional.of(1));

			try (CheckpointWriter w = cf.createWriter(cpFs, new Path(cpDir + "cp_1"), conf)) {
				w.writeUnsorted(kv.entrySet(), is, is, is);
			}
			...
			kv.clear();
			kv.put(Tuple2.of(0, 1), Optional.of(2));
			...
			assertEquals(Optional.of(0),
					checkpointManager.lookupKey(AbstractCheckpointWriter.serialize(Tuple2.of(0, 0), is, is), is));
			assertEquals(Optional.of(2),
					checkpointManager.lookupKey(AbstractCheckpointWriter.serialize(Tuple2.of(0, 1), is, is), is));
		...
		try (HdfsCheckpointManager scanner = new HdfsCheckpointManager(cpFs, cpDir, localFs, tmpDir, ipMap,
				conf)) {
			assertEquals(Optional.of(0),
					scanner.lookupKey(AbstractCheckpointWriter.serialize(Tuple2.of(0, 0), is, is), is));
			assertEquals(Optional.of(2),
					scanner.lookupKey(AbstractCheckpointWriter.serialize(Tuple2.of(0, 1), is, is), is));
		}
	}
	...
	private static void runTest(CheckpointerFactory cf, boolean keepLocal, boolean copyBack, int compactEvery,
		...
		HdfsKvState<Integer, Integer, String> state = new HdfsKvState<>(
				backend,
				"test",
				conf,
				IntSerializer.INSTANCE,
				IntSerializer.INSTANCE, vs, 0, 0,
				fs,
				cpDir,
				fs,
				tmpDir,
