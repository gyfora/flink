...
public class LazyDbKvState<K, V> implements KvState<K, V, DbStateBackend>, CheckpointNotifier {
	 ...
	 * job starts.
	 */
	public LazyDbKvState(String kvStateId, int subTaskIndex, Connection con, DbBackendConfig conf,
			TypeSerializer<K> keySerializer,
			TypeSerializer<V> valueSerializer, V defaultValue) throws IOException {
		this(kvStateId, subTaskIndex, con, conf, keySerializer, valueSerializer, defaultValue, -1);
	}

	/**
	 * Initialize the {@link LazyDbKvState} from a snapshot.
	 */
	public LazyDbKvState(String kvStateId, int subTaskIndex, Connection con, final DbBackendConfig conf,
			TypeSerializer<K> keySerializer,
	...
	public void notifyCheckpointComplete(long checkpointId) throws Exception {
		...
		if (compactEvery > 0 && subTaskIndex == 0 && checkpointId % compactEvery == 0) {
	...
	private static class DbKvStateSnapshot<K, V> implements KvStateSnapshot<K, V, DbStateBackend> {
		...
		public LazyDbKvState<K, V> restoreState(final DbStateBackend stateBackend,
			...
			int subTaskIndex = stateBackend.getEnvironment().getIndexInSubtaskGroup();
			...
			// Restore the KvState
			LazyDbKvState<K, V> restored = new LazyDbKvState<K, V>(kvStateId, subTaskIndex,
