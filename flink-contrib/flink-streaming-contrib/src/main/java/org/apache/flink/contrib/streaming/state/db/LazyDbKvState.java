 ...
 * 
 */
public class LazyDbKvState<K, V> extends OutOfCoreKvState<K, V, DbStateBackend> {
	...
	public LazyDbKvState(DbStateBackend backend, String kvStateId, boolean compact, ShardedConnection cons,
		...
		this(backend, kvStateId, compact, cons, conf, keySerializer, valueSerializer, defaultValue, 0, 1);
	}
	 ...
	 */
	public LazyDbKvState(DbStateBackend backend, String kvStateId, boolean compact, ShardedConnection cons,
			final DbBackendConfig conf, TypeSerializer<K> keySerializer, TypeSerializer<V> valueSerializer,
			V defaultValue, long lastCheckpointTs, long currentTs) throws IOException {

		super(backend, kvStateId, conf, keySerializer, valueSerializer, defaultValue, 0, lastCheckpointTs, currentTs);
	...
	@Override
	public KvStateSnapshot<K, V, DbStateBackend> snapshotStates(Collection<Entry<K, Optional<V>>> modifiedKVs,
			long checkpointId, long timestamp) throws IOException {
		evictModified(modifiedKVs, lastCheckpointId, lastCheckpointTs, timestamp);

		return new DbKvStateSnapshot<K, V>(stateId, timestamp);
	...
	public void evictModified(Collection<Entry<K, Optional<V>>> KVsToEvict, long lastCheckpointId,
		...
		for (Entry<K, Optional<V>> state : KVsToEvict) {
			batchInsert.add(state, currentTs);
		}
		batchInsert.flush(currentTs);
	}

	@Override
	public Optional<V> lookupLatest(final K key, final byte[] serializedKey) throws IOException {
	...
	public void compact(long from, long to) throws IOException {
		if (compact) {
			executor.execute(new Compactor(from, to));
		}
	}
	...
	private class Compactor implements Runnable {
		...
		public Compactor(long from, long to) {
			this.from = from;
			this.to = to;
		}
	...
	private static class DbKvStateSnapshot<K, V> implements KvStateSnapshot<K, V, DbStateBackend> {
		...
		public LazyDbKvState<K, V> restoreState(final DbStateBackend stateBackend,
			...
			LazyDbKvState<K, V> restored = new LazyDbKvState<K, V>(stateBackend, kvStateId, compactor,
					...
					defaultValue, checkpointTimestamp, recoveryTimestamp + 1);
